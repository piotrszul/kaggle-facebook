...
case class HiveSQLDistSummaryWithParam[T](baseName:String, query:String, param:String, version:Int=1)(implicit hc:HiveContext, implicit val stag:ClassTag[T]) {
  ...
  val stats = List("count","avg", "max", "min", "dev", "median", "pc90")
  
  lazy val result = {
    val actualQuery = String.format(query,param)
    val summaryQuery = String.format("SELECT key,count(*) as count,avg(cnt) as avg, max(cnt) as max, min(cnt) as min," 
        + "stddev_samp(cnt) as dev, percentile(cnt,0.5) as median, percentile(cnt,0.9) as pc90" 
        //+ "(avg(cnt^3)-3*avg(cnt)*stddev_samp(cnt)^2 -avg(cnt)^3)/stddev_samp(cnt)^3 as skew" 
        +" FROM(%s) as tmp GROUP BY key", actualQuery);
